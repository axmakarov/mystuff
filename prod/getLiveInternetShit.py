...
logging.basicConfig(format = u'%(filename)s[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s', level = logging.DEBUG, filename = u'D:\\liGetterLog1_{0}.log'.format(logdate))
...
for item in domains:
    ...
    for searchEngine in searchEngines:
        ...
        for date in dates:
            ...
            data = getLiveinternetResponse(domain,searchEngine,date,1)
            ...
            if len(data) > 3:
                ...
                logging.warning(u'Недостаточно данных')
            if len(data) == 103:
                logging.info(u'Есть следующая страница')
                for pageCounter in xrange(2,11):
                    logging.debug(u'Делаем страницу {0}'.format(pageCounter))
                    data2 = getLiveinternetResponse(domain,searchEngine,date,pageCounter)
                    if data2 == data or len(data2) == 3:
                    ...
                    if len(data) != 103:
                        ...
                        logging.info(u'Страница не последняя. Продолжаем...')
